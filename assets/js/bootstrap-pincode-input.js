		...
		$.extend(Plugin.prototype, {
				...
				buildInputBoxes: function () {
					...
					if(this._isTouchDevice()){
						...
						var input = $('<input>').attr({'type':'number', 'style':'-webkit-text-security:disc','pattern': "[0-9]*", 'placeholder':touchplaceholders,'inputmode':"numeric",'maxlength':this.settings.inputs,'autocomplete':'off'}).addClass('form-control pincode-input-text').appendTo(wrapper);
			        	...
			        	for (var i = 0; i <  this.settings.inputs; i++) {
			        		...
			        		var input = $('<input>').attr({'type':'text','maxlength':"1",'style':'-webkit-text-security:disc !important','autocomplete':'off','placeholder':(placeholders[i] ? placeholders[i] : undefined)}).addClass('form-control pincode-input-text').appendTo(this._container);
			        		if(this.settings.hidedigits){
										// hide digits
			        			input.attr('type','password');
			        		}else{
