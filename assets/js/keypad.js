(function ($) {
    ...
    Keypad.prototype.set = function (i){
        this.number = i;
    };
    ...
    Keypad.prototype._bindEvents = function () {
        ...
        instance.on('click', 'button', function (e) {
            ...
            else if(cmd == self.CMD_SEND){
                console.log(self.number);
                $('form#myform'+self.number).submit();
              //  self._send();
    ...
    Keypad.prototype._appendValue = function (pressedVal) {
        ...
        if(val.length<4)
        {
            newVal = val.substring(cursorPos, val.length) + pressedVal + val.substring(0, cursorPos);
            this.el.val(newVal);
        } 
    };
    ...
    '                    <td><button type="button" class="text-sm" cmd="send">Back</button></td>   ' +
    '                </tr>   ' +
    '            </tbody>   ' +
    '        </table>   ' +
    '    </div>   ';
    ...
    $.fn.keyPad = function (options, number) {
